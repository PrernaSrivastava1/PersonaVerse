...
export default function AnimeCard({ result }: { result: PersonalityResult }) {
  return (
    <motion.section initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
      className="glass-strong" style={{ padding: '2rem', marginBottom: '1.5rem', borderImage: 'linear-gradient(135deg, #E6A817, #FFD700) 1', borderWidth: 2, borderStyle: 'solid' }}>
      <p style={{ fontSize: '0.65rem', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '0.5rem', textAlign: 'center', color: '#E6A817' }}>🎌 YOUR ANIME CHARACTER MATCH</p>
      ...
      <p style={{ textAlign: 'center', fontSize: '0.85rem', marginBottom: '1rem', color: '#FFD700' }}>from {result.animeMatch.series}</p>
      <p style={{ textAlign: 'center', fontSize: '0.9rem', maxWidth: 480, margin: '0 auto', color: '#F5F0E1' }}>{result.animeMatch.reason}</p>
