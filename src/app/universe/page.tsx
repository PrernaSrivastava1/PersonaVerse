...
const PLANETS: { key: PlanetPath; title: string; desc: string; color: string }[] = [
  { key: 'mind', title: '🧠 Planet of Mind', desc: 'Logic, strategy & intellectual puzzles', color: '#FFD700' },
  { key: 'emotion', title: '❤️ Planet of Emotion', desc: 'Feelings, relationships & empathy', color: '#E6A817' },
  { key: 'chaos', title: '🔥 Planet of Chaos', desc: 'Adventure, risk & wild scenarios', color: '#FFCC00' },
];
    ...
    <div className="page-container">
      ...
      <div className="content-overlay" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 1rem' }}>
        ...
        >
          ...
          <p style={{ fontSize: '0.85rem', color: '#A89968' }}>
        ...
        <div style={{ marginTop: 'auto', marginBottom: '2rem', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '1rem', pointerEvents: 'auto' }}>
          {PLANETS.map(({ key, title, desc, color }, i) => (
            <motion.button
              ...
              className={selected === key ? 'glass glow-gold' : 'glass-subtle'}
              style={{
            ...
            >
              <h3 style={{ fontSize: '0.95rem', fontWeight: 600, marginBottom: '0.35rem', color: selected === key ? color : '#F5F0E1' }}>
                {title}
              </h3>
              <p style={{ fontSize: '0.72rem', color: '#6B6340', lineHeight: 1.4 }}>{desc}</p>
            </motion.button>
          ))}
        </div>

        {selected && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', fontSize: '1.05rem', marginBottom: '1.5rem', color: '#FFD700' }}>
